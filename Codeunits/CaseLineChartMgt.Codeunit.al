codeunit 90003 "Case Line Chart Mgt."
{
    ...
    local procedure AddColumns(var BusChartBuf: Record "Business Chart Buffer"; CaseHoursChartSetup: Record "Case Hour Chart Setup")
    var
    ...
    local procedure AddMeasures(var BusChartBuf: Record "Business Chart Buffer"; CaseHoursChartSetup: Record "Case Hour Chart Setup")
    var
        UserSetup: Record "User Setup";
        CaseHoursChartSetup2: Record "Case Hour Chart Setup";
        ChartType: Option;
        ...
        if CaseHoursChartSetup.Type in [CaseHoursChartSetup.Type::Total, CaseHoursChartSetup.Type::Pie] then begin
            ...
            if UserSetup.FindSet then
                repeat
                    if CaseHoursChartSetup2.Get(UserSetup."User ID") and CaseHoursChartSetup2."Show in Chart" then
                        with BusChartBuf do
                            BusChartBuf.AddMeasure(UserSetup."Resource No.", '', "Data Type"::Decimal, "Chart Type"::Line);
                until UserSetup.Next = 0;
        end;
    end;
    ...
    local procedure SetCaseLineTotalFilter(CaseHourChartSetup: Record "Case Hour Chart Setup"; var CaseLine: Record "Case Line"; ResourceNo: Code[20]; PType: Integer)
    var
        Expr: Text[1];
    begin
    ...
    local procedure SetCaseLineAccumulatedFilter(var CaseLine: Record "Case Line"; ResourceNo: Code[20]; PDate: Date)
    begin
        SetCaseLineFilter(CaseLine, ResourceNo, 0);
        CaseLine.SetRange(Date, CalcDate('<-CY>', PDate), PDate);
    end;

    local procedure SetCaseLineFilter(var CaseLine: Record "Case Line"; ResourceNo: Code[20]; Chargeable: Option Yes,No,Both)
    begin
