codeunit 50007 "Delete Service Invoice"
{
        ...
        if ServiceLine.FindSet then begin
            ...
            repeat
                if ServiceLine."No." <> xServiceLine."No." then begin
                    ...
                    LastInvoicedtoDate := ServiceLine."SCL Invoiced to Date";
                end else
                    if LastInvoicedtoDate > ServiceLine."SCL Invoiced to Date" then
                        LastInvoicedtoDate := ServiceLine."SCL Invoiced to Date";
    ...
    begin
        if ServContractHeader.Get(ServContractHeader."Contract Type"::Contract, ServHeader."Contract No.") then begin
            ...
            ServContractHeader."Next Invoice Date" := ServHeader."Serv. Contr. Next Inv. Date";
            ServContractHeader."Last Invoice Date" := ServHeader."Serv. Contr. Last Inv. Date";
            ServContractHeader."Next Invoice Period Start" := ServHeader."SC Next Inv. Period Start";
            ServContractHeader."Next Invoice Period End" := ServHeader."SC Next Inv. Period End";
            ServContractHeader."Last Invoice Period End" := ServHeader."SC Last Inv. Period End";
    ...
    begin
        ...
        if ServiceLine.FindSet then
            repeat
                if ServiceContractLine.Get(ServiceContractHeader."Contract Type", ServiceContractHeader."Contract No.", ServiceLine."SCL Line No.") then begin
                    ServiceContractLine.Validate("New Line", ServiceLine."SCL New Line");
                    ServiceContractLine.Modify(true);
                end;
            until ServiceLine.Next = 0;
    end;
