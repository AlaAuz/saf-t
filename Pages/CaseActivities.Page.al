page 50003 "Case Activities"
{
    ...
    begin
        ...
        SetFilter("Resource Filter", StrSubstNo('@*%1*', UserSetup."Resource No.")); // Used STRSUBSTNO because of bug

        if UserSetup.Consultant then begin
            SetRange("Waiting for Filter", "Waiting for Filter"::Consultant);
            SetRange("Consultant Filter", UserSetup."User ID");
        end else
            if UserSetup.Developer then begin
